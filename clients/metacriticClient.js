...
async function getGameScore(game) {
  let score = 0
...
async function getMetaCriticId(titleToSearch, gameId) {
  const browser = await puppeteer.launch({ headless: true })
  const page = await browser.newPage()
  await page.setDefaultNavigationTimeout(15000)
  const url = `https://www.metacritic.com/search/game/${titleToSearch}/results?plats[3]=1&search_type=advanced`
  let metaCriticId
  try {
    await page.goto(url)
